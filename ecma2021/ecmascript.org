...
*** 6.2.3.1.1 Await Fulfilled Functions
  Await Fulfilled Functions是匿名内建函数，是Await规范的一部分，用于将promise fulfillment值
作为普通完结派发到调用者。每个Await fulfilled函数又一个内置的\[\[AsyncContext\]\]
  当Await fulfilled函数被调用在value上时，下列步骤会被执行
...
6. 继续asyncContext上已经挂起的评估，用NormalCompletion(value)作为这个挂起的操作的结果。
7. Assert：当到达这步的时候，asyncContext已经被从execution context stack移除，而且
