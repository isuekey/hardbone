...
1. 让keys是新的空List
2. 遍历O的每个自有性质的键P，如果P是数组索引，按照数字升序，做
    a. 追加P到keys中
3. 遍历O的每个自有性质的键P，如果Type(P)是字符串，且不是数组索引，按照创建
   顺序升序，做
    a. 追加P到keys中
...
22. 如果argumentsObjectNeeded是true，那么
    ...
    b. 否则
        i. 注意：
       ii. 执行!env.CreateMutableBinding(paramName, false)
    c. 如果alreadyDeclared是false，那么
        i. 执行!env.CreateMutableBinding(paramName, false)
    d. 如果alreadyDeclared是false，那么
        i. 执行!env.CreateMutableBinding(paramName, false)
    e. 如果alreadyDeclared是false，那么
    f. 如果alreadyDeclared是false，那么
23. 
24. 
25. 
26. 
