...
ScriptRecord封存着被评估的脚本信息。每个scriptRecord含有列在表39的字段。

| 字段名               | 值类型                           | 含义                                                            |
| \[\[Realm]]          | RealmRecord 或者 undefined       | 创建本脚本的圈。没有被指定时是undefined                         |
...
***** 16.2.1.6.2 GetExportedNames(\[exportStartSet]) Concrete Method
