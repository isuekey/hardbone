...
***** 9.1.1.1.1 HasBinding(N)
  DeclarativeEnvironmentRecord envRec的HasBinding方法接受参数String值N。明确
参数标识是否已经绑定在记录中。被调用时执行下列步骤：
1. 如果envRec有标识的名字与N一致，返回true。
2. 返回false。
***** 9.1.1.1.2 CreateMutalbeBinding(N, D)
  DeclarativeEnvironmentRecord envRec的CreateMutableBinding方法接受参数String值N和
Boolean值D。创建一个可变的名字为N的未初始化绑定。这个EnvironmentRecord必须要
...
***** 9.1.1.1.3 CreateImmutableBinding
  DeclarativeEnvironmentRecord envRec的CreateImmutableBinding方法接受参数String值N
和Boolean值S。创建一个不变的名字为N的未初始化绑定。这个EnvironmentRecord必须
...
***** 9.1.1.1.4 InitializeBinding
  DeclarativeEnvironmentRecord envRec的InitializeBinding方法接受参数String值N和
ECMAScript语言值V。用于将绑定N的值替换为V。未初始化的绑定N必须已经存在。
被调用时执行下列步骤：
...
***** 9.1.1.1.5 SetMutableBinding
  DeclarativeEnvironmentRecord envRec的SetMutableBinding方法接受参数String值N、
ECMAScript语言值V和Boolean值S。尝试将绑定N的当前值替换为参数V的值。绑定N通常
已经存在，但在罕见的情况下也可能没有。如果绑定N是不变绑定，且S是true，会
投掷一个TypeError异常。被调用时执行下列步骤：
1. 如果envRec没有绑定N，那么
...
***** 9.1.1.1.6 GetBindingValue
  DeclarativeEnvironmentRecord envRec的GetBindingValue方法接受参数String值N和
Boolean值S。返回envRec上绑定N的值。如果绑定存在但是没有初始化，会投掷
ReferenceError异常，无论S的值。被调用时执行下列步骤：
1. Assert：envRec有绑定N
2. 如果envRec的绑定N没有初始化，投掷ReferenceError异常
3. 返回envRec的绑定N的值。
***** 9.1.1.1.7 DeleteBinding
  DeclarativeEnvironmentRecord envRec的DeleteBinding方法接受参数String值N。只能删除
...
**** 9.1.1.2 Object Environment Records
  每个ObjectEnvironmentRecord被联结到对象，这个对象称为绑定对象。
** 25.1
